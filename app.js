...
const yearFiltering=inventors.filter(inventor => inventor.year >= 1500 && inventor.year< 1600)
console.table(yearFiltering);
  // Array.prototype.map()
...
const whoIsOlder= inventors.sort((firstPerson, secondPerson)=>
    firstPerson.year>secondPerson.year ? 1 : -1
)
console.table(whoIsOlder);
...
const sortOfOldest= inventors.sort(function(firstPerson, secondPerson){
    const firstMan= firstPerson.passed-secondPerson.year;
    const secondMan= secondPerson.passed-secondPerson.year;
    return firstMan>secondMan?1:-1;
})
